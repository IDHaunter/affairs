  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Text(context.watch<GlobalViewModel>().takeDataS), //смотрит за моделью
        Text(Provider.of<GlobalViewModel>(context, listen: true).takeDataS), //смотрит за моделью
        const SizedBox(
          height: 20,
        ),
        const SomeDataS(),
  ...
  Widget build(BuildContext context) {
    return TextField(
        onChanged: (newData) =>
            ...
            Provider.of<GlobalViewModel>(context, listen: false).putDataS(newData)); //пишет в модель
